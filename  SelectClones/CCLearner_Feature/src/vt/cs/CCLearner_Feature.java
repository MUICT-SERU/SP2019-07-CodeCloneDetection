...
public class CCLearner_Feature {
    ...
    public static Connection connFalse;
	public static Statement stat;
    public static Statement statFalse;
	public static ResultSet res;
	public static ResultSet resFalse;
	public static ResultSetMetaData rsmd;
    public static ResultSetMetaData rsmdFalse;
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            String sql = "select A.path as path1, A.name as name1, A.startline as startline1, A.endline as endline1, A.type as type1, "
                + "B.path as path2, B.name as name2, B.startline as startline2, B.endline as endline2, B.type as type2, M.syntactic_type, M.similarity_token "
                + "from "
                + "( "
                + "select * from clones "
...
//                }
        ...
        //Generate False clone
        String sqlFalse = "select A.path as path1, A.name as name1, A.startline as startline1, A.endline as endline1, A.type as type1, "
        + "B.path as path2, B.name as name2, B.startline as startline2, B.endline as endline2, B.type as type2, M.syntactic_type, M.similarity_token "
        + "from "
        + "( "
        ...
        connFalse = DriverManager.getConnection(connURL, userName, password);
        connFalse.setAutoCommit(false);
        statFalse = connFalse.createStatement();
        statFalse.setFetchSize(500);
        resFalse = statFalse.executeQuery(sqlFalse);
        ...
        while (resFalse.next()) {
            falseClone = "";
            String[] rowFalse = new String[rsmdFalse.getColumnCount()];
            for (int indexFalse = 0; indexFalse < rsmdFalse.getColumnCount(); indexFalse++) {
                rowFalse[indexFalse] = String.valueOf(resFalse.getObject(indexFalse + 1));
                falseClone += (rowFalse[indexFalse]+",");
            }
            double num = generator.nextDouble() * (0.5);
            falseClonesList.add(num+","+falseClone);
        }

        Collections.sort(falseClonesList);

        for (int j = 0; j < 22663; j++) {
            String[] del = falseClonesList.get(j).split(",");
            String value = del[1];
            for (int k=2; k<del.length; k++){
                value += ","+del[k];
            }
            false_writer.append("\n" + value);
        }
